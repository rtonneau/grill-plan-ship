...
│   └── lib/                 ← Shared helpers (session-store, templates, write-target, write-payload, ticket-queue, github) + tests
...
│   ├── 03-ticket-name/
│   └── 04-ticket-name/
├── .session-config.json  ← Machine state (session ID, status, tickets, `git` branch/PR on GitHub projects)
