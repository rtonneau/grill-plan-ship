...
│   └── lib/                 ← Shared helpers (session-store, history, ticket-lookup, templates, write-target, write-payload, ticket-queue, github) + tests
...
│   ├── 03-ticket-name/
│   └── 04-ticket-name/
├── .session-config.json  ← Machine state (session ID, `current_phase`, `history` timeline events, usage, and `git` branch/PR on GitHub projects)
