...
1. Runs `node $CLAUDE_PLUGIN_ROOT/scripts/status.js`, which:
   - Lists every session under `.work/sessions/` with its feature name, creation date, `finishedAt`, `branch` and `prUrl` (GitHub sessions; `null` otherwise), and `phase` (computed from the session's files: `grill`, `plan-not-started`, `plan`, `ship`, `finish-pending`, `plan-complete` or `finished`; old `status` / `phases_completed` config fields are ignored).
